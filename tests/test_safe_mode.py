...
def test_intercept_patches_only_listed_apis(tmp_path, monkeypatch, register_target):
    ...
    fake = types.ModuleType("flowcoder_office_tools.fake_api")

    def real_call(x):
        return f"REAL:{x}"

    fake.call = real_call
    sys.modules["flowcoder_office_tools.fake_api"] = fake

    # 인터셉트 대상 추가
    register_target("fake", "flowcoder_office_tools.fake_api", "call")
    ...
    with safe_mode.intercept("case_x", apis=["fake"]):
        from flowcoder_office_tools import fake_api

        result = fake_api.call("hi")
        assert isinstance(result, dict)
        assert result.get("_safe") is True
        assert result.get("qualname") == "flowcoder_office_tools.fake_api.call"
...
def test_intercept_returns_cached_when_present(tmp_path, monkeypatch, register_target):
    ...
    fake = types.ModuleType("flowcoder_office_tools.fake_api2")
    fake.call = lambda x: "REAL"
    sys.modules["flowcoder_office_tools.fake_api2"] = fake
    register_target("fake2", "flowcoder_office_tools.fake_api2", "call")

    # 캐시 파일 미리 작성
    case_id = "case_y"
    key = safe_mode._key("flowcoder_office_tools.fake_api2.call", ("hi",), {})
    ...
    with safe_mode.intercept(case_id, apis=["fake2"]):
        from flowcoder_office_tools import fake_api2
