...
def test_draft_email_returns_three_options(monkeypatch):
    def fake_chat(messages, **k):
        return '[{"option": 1, "subject": "s1", "body": "b1"}, {"option": 2, "subject": "s2", "body": "b2"}, {"option": 3, "subject": "s3", "body": "b3"}]'
...
def test_draft_email_handles_safe_fallback(monkeypatch):
    from core.ai import client
    monkeypatch.setattr(client, "chat", lambda m, **k: "[SAFE-FALLBACK]")
