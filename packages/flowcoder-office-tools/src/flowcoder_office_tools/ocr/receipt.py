...
# R2-H2: ``%m/%d/%Y`` 와 ``%d/%m/%Y`` 는 의도적으로 제외한다 — ``05/06/2026``
# 같은 입력이 미국식(5월 6일)인지 유럽식(6월 5일)인지 구분 불가하고, 순서상
# 첫 번째에 해당하는 포맷이 silent하게 이긴다 (silent misparse 위험).
...
def _normalize_date(value: str) -> str:
    ...
    지원 포맷 (순서대로 시도):
        ...
        50 이상은 1900년대로 해석), ``26/05/01``.

    의도적으로 제외:
        ``MM/DD/YYYY`` (미국식)와 ``DD/MM/YYYY`` (유럽식)는 ``05/06/2026``
        같은 입력에서 silent misparse 가능하므로 둘 다 거절한다 (R2-H2).
